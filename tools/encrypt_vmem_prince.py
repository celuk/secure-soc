...
def set_nibble(val, pos, nib):
    mask = 0xF << (pos * 4)
    return (val & ~mask) | ((nib & 0xF) << (pos * 4))

def shift_rows(mp):
    out = 0
    mapping = [
        (15, 15), (14, 10), (13, 5), (12, 0),
        (11, 11), (10, 6), (9, 1), (8, 12),
        (7, 7), (6, 2), (5, 13), (4, 8),
        (3, 3), (2, 14), (1, 9), (0, 4),
    ]
...
def process_vmem_file(input_file):
        ...
        for line in f:
            ...
            for i in range(0, len(data_bytes_str), 4):
                ...
                for j, byte_str in enumerate(word_bytes):
                    word_val |= int(byte_str, 16) << (j * 8)

                byte_offset = current_address + i
                word_address = BASE_ADDR + byte_offset

                encrypted_word = prince_encrypt_word(word_address, word_val, KEY, NONCE)

                for j in range(4):
                    encrypted_output_bytes.append(f'{(encrypted_word >> (j * 8)) & 0xFF:02X}')

    print('@00000000')
    bytes_per_line = 16
    for i in range(0, len(encrypted_output_bytes), bytes_per_line):
        print(' '.join(encrypted_output_bytes[i:i + bytes_per_line]))

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--file', '-f', type=str, required=True)
    args = parser.parse_args()
    process_vmem_file(args.file)
